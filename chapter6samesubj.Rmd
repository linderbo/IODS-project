...
The anova gives us p =0.28. This tells us that no significant differences in relative weight gains exist between groups. Thus there is no need to test this between individual groups.

## MABS chapter 9 analyses on BPRS data.

We read the BPRS data in long form, factorize `subject`and `treatment` variables. and have a look at the data
...
The estimated slope is still -2.27, but std. error is slightly lower 0.21 vs. 0.25. 
...
BPRS_rism <- lmer(bprs ~ week + treatment + (week | subject), data = BPRSL, REML = FALSE)
...
We get a chi squared of 7.27 with 2 degrees of freedom, and a p of 0.026 indicating that this model with random intercepts and slopes for different subjects is a better fit than the previous model with only random intercepts.
...
BPRS_rismi <- lmer(bprs ~ week * treatment + (week | subject), data = BPRSL, REML = FALSE)
...
The model with an interaction of time and treatment is not significantly better than the previous one. (p = 0.07)
...
ggplot(BPRSL, aes(x = week, y = FittedBPRS, group = interaction(treatment,subject))) +
  geom_line(aes(colour = treatment)) +
  scale_x_continuous(name = "Time (weeks)", breaks = seq(0, 8, 1)) +
